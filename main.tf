resource "aws_cloudtrail" "default" {
  ...
  cloud_watch_logs_role_arn     = var.enable_cloudwatchlogs ? aws_iam_role.cloudwatch_logs[0].arn : null
  tags                          = var.tags
...
resource "aws_s3_bucket" "default" {
  ...
  bucket        = var.name
  force_destroy = tobool(var.force_destroy)
  tags          = var.tags
}
...
resource "aws_cloudwatch_log_group" "cloudtrail" {
  ...
  kms_key_id        = var.kms_key_id
  retention_in_days = tonumber(var.log_retention_days)
  tags              = var.tags
}
...
resource "aws_iam_role_policy" "cloudwatch_logs_organizations" {
  count  = var.enable_cloudwatchlogs && var.is_organization_trail ? 1 : 0
...
data "aws_iam_policy_document" "cloudwatch_logs_role_organizations" {
  ...
  statement {
    sid       = "AWSCloudTrailOrganizationsCreateStream"
    effect    = "Allow"
  ...
  statement {
    sid       = "AWSCloudTrailOrganizationsPutEvents"
    effect    = "Allow"
